...
class PatientVariantScreen extends React.Component {
  ...
  handleCreateDraftStatement(newStatement) {
    ...
    console.log(`+ newStatement ${JSON.stringify(newStatement)}`);

    actions.createDraftStatement(newStatement);
    const lastQuery = last(newStatement.queries);
    this.handleQuerySelection(lastQuery.key);
  }
...
/>
            ...
            <Row type="flex" className={style.descriptionPatient}>
              <Col>
                <IconKit size={16} icon={ic_assignment_ind} />
                {' '}
                {patient.details.mrn}
              </Col>
              <Col>
                <IconKit size={16} icon={ic_location_city} />
                {' '}
                {patient.organization.name}
              </Col>
              <Col>
                <IconKit size={16} icon={ic_folder_shared} />
                {' '}
