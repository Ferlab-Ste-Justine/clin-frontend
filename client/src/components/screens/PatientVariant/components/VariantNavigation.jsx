...
class VariantNavigation extends React.Component {
    ...
    if (onEditCallback) {
      ...
      if (filterChange && query) {
        ...
        if (!filterChange.remove) {
          ...
          if (!updated) {
            const andNotOperator = find(
              updatedQuery.instructions,
              (instruction) => (
                instruction.type === INSTRUCTION_TYPE_OPERATOR && instruction.data.type === OPERATOR_TYPE_AND_NOT
              ),
            );
            if (!andNotOperator) {
  ...
  renderFilterType(categoryData) {
    ...
    const activeFilterForActiveQuery = activeQueryData
      ? find(activeQueryData.instructions, (q) => q.data.id === currentActiveFilterId)
      : null;
      ...
      case FILTER_TYPE_GENERIC:
        return (
          <GenericFilter
            ...
            data={(activeFilterForActiveQuery
              ? activeFilterForActiveQuery.data
              : GenericFilter.structFromArgs(currentActiveFilterId))}
            dataSet={dataSet}
            config={config}
      ...
      case FILTER_TYPE_SPECIFIC:
        return (
          <SpecificFilter
            ...
            data={(activeFilterForActiveQuery
              ? activeFilterForActiveQuery.data
              : SpecificFilter.structFromArgs(currentActiveFilterId))}
            dataSet={dataSet}
      ...
      case FILTER_TYPE_NUMERICAL_COMPARISON:
        return (
          <NumericalComparisonFilter
            ...
            data={(activeFilterForActiveQuery
              ? activeFilterForActiveQuery.data
              : NumericalComparisonFilter.structFromArgs(currentActiveFilterId))}
            facets={data}
            dataSet={dataSet}
            config={config}
            onEditCallback={this.handleFilterChange}
      ...
      case FILTER_TYPE_GENERICBOOL:
        return (
          <GenericBooleanFilter
            ...
            data={(activeFilterForActiveQuery
              ? activeFilterForActiveQuery.data
              : GenericBooleanFilter.structFromArgs(currentActiveFilterId))}
            dataSet={categoryData.search && Object.keys(categoryData.search).length > 0
              ? Object.keys(categoryData.search).reduce((accumulator, keyName) => {
      ...
      case FILTER_TYPE_COMPOSITE:
        return (
          <CompositeFilter
            ...
            data={(activeFilterForActiveQuery
              ? activeFilterForActiveQuery.data
              : CompositeFilter.structFromArgs(currentActiveFilterId))}
            facets={data}
            dataSet={dataSet}
            config={config}
            onEditCallback={this.handleFilterChange}
