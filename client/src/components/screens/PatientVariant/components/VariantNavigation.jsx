...
class VariantNavigation extends React.Component {
  ...
  handleGeneSearch(query) {
    ...
    } else if (this.searchGeneQuery !== query) {
  ...
  render() {
    ...
    /*     const autocompletesGene = searchGeneValue !== '' ? searchGeneResults.filter(group => group.label !== '').map((group) => {
      autocompletesGeneCount += group.matches.length;
      return (
        <AutoComplete.OptGroup key={group.id} disabled label={(<Typography.Text strong className="label">{group.label}</Typography.Text>)}>
          {group.matches.map(match => (
            <AutoComplete.Option key={match.id} value={JSON.stringify(match)} className="value">
          ...
          if (category.filters && category.filters.length > 0) {
            ...
            return (
              ...
              >
                ...
                { activeFilterId === null && !searchSelection.category && category.filters.map(f => (f.search && f.label !== 'filter_gene_symbol') && (
                  <Menu.SubMenu
                    key={f.id}
                    title={
