...
import ConfirmCancelModal from '../PatientSubmission/components/ConfirmCancelModal';
import SubmissionModal from '../PatientSubmission/components/SubmissionModal';

import ClinicalInformation from './components/ClinicalInformation';

import '../PatientSubmission/style.scss';
...
function PatientSubmissionScreen(props) {
  ...
  const validate = () => {
    ...
    console.log('is valid : ', valid, ' state : ', state.valid);
  ...
  const saveSubmission = (data, submitted = false) => {
    const { actions, currentPatient } = props;
    ...
    allAnalysis.forEach((analysis) => {
      batch.serviceRequests.push(
        new ServiceRequestBuilder()
          ...
          .withRequester(state.selectedPractitioner ? state.selectedPractitioner.id : '')
          .withSubject(currentPatient ? currentPatient.id : '')
      ...
      batch.clinicalImpressions.push(
        new ClinicalImpressionBuilder()
          .withId(get(localStore, 'clinicalImpression.id', '23'))
          .withSubmitted(submitted)
          .withSubject(currentPatient ? currentPatient.id : '')
          .withAge(ageInDay)
          .withAssessorId(userRole ? userRole.id : '')
          .build(),
      );
    });
  ...
  const handleSubmission = () => {
    ...
    saveSubmission(form.getFieldsValue())
  };
  ...
  const { actions, app, clinicalImpression, patient, serviceRequestCode } = props;
  const { fmhResources, hpoResources, isSubmitting, submitFailed } = state;
  const userRole = {
    code: [],
    id: patient.generalPractitioner.length > 0 ? patient.generalPractitioner[0].reference.split('/')[1] : '',
  }

  const { Title } = Typography;
  const pageContent = (<ClinicalInformation
  ...
  return (
    <Layout>
      ...
      <ConfirmCancelModal
        ...
        onQuit={() => handleCancel()}
        onSaveAndQuit={() => {
          form.validateFields().then(saveSubmission(data, false));
        }}
